...
void specialKeyboardFuncUp(int key, int x, int y);
...
void server_update(int value){
	...
	*/
	...
	if ((out[0] == '{') && (out[1] == '}' || ((out[1] == '"')&& (out[2] == '0' || out[2] == '1' || out[2] == '2' || out[3] == '3') )))
	{
		unsigned pos = out.find("`");
		out = out.substr(0, pos);
		gs.parseJSONString(out);
		gs.shootChanged(0);
		gs.shootChanged(1);
		gs.shootChanged(2);
		gs.shootChanged(3);
		std::cout << gs.getJSONStringFull();
		recvValid = true;
	}
	...
	if (recvValid)
	{
		...
		player_list = gs.getObjectByType("player");
		/*
...
{/*
	...
	state.addObject(obj4);
	state.addObject(obj5);

	state2.addObject(obj6);
	state2.addObject(obj7);
	state2.addObject(obj8);
	state2.addObject(obj9);
	state2.addObject(obj10);

	obj1->postTrans(mat4(-1.99999));
	obj2->postTrans(mat4(-2.99999));
	obj3->postTrans(mat4(-3.99999));
	obj4->postTrans(mat4(-4.99999));
	obj5->postTrans(mat4(-5.99999));
	...
	state.posChanged(3);
	state.posChanged(4);
	string temp = state.getJSONStringFull();
	state2.parseJSONString(temp);
	state2.camChanged(0);
	state2.camChanged(1);
	state2.camChanged(2);
	state2.camChanged(3);
	...
	cout << temp << endl;
	system("pause");
	cout << state2.getJSONStringFull() << endl;
	system("pause");
  ...
  glutSpecialUpFunc(specialKeyboardFuncUp);
...
void keyboard(unsigned char key, int, int){
	...
	case 1:
		...
		//cout << glutGetModifiers() << " " << key << endl;
		if (key == 'a' || key == 'A'){
			if (!(keyState & 1)){
				keyState = keyState | 1;
...
void specialKeyboardFunc(int key, int x, int y){
	...
	case 1:
		if (glutGetModifiers() ==  GLUT_ACTIVE_SHIFT){
			shift_down = true;		
			if (!(keyState & 1 << 9)){
				keyState = keyState | 1 << 9;
			}
			cout << shift_down << endl;
			if (sprint_up >= 10){
			//	testSound[1]->Play(FMOD_CHANNEL_FREE, 0, &channel);
				//scene->jump(playerID);
			}
...
void specialKeyboardFuncUp(int key, int x, int y){
	...
	case 0:
		break;
	case 1:
		shift_down = ((GetKeyState(VK_SHIFT) & 0x80) > 0);
		cout << shift_down << endl;
		keyState = keyState & ~(1 << 9);
...
void initialize(int argc, char *argv[])
{
	...
	for (int i = 0; i < 4; i++){
		if (i % 2){
			...
			player0->setFog(fog);
			player0->setType("player");
			player_list.push_back(player0);
			gs.addObject(player0);
		}
		else{
			Mesh* player0 = new Mesh();
